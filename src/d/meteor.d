...
import std.conv;
    ...
    static const int N_PARITY = 2;

    static const int GOOD = 0;
    static const int BAD = 1;
    static const int ALWAYS_BAD = 2;

    static const int OPEN    = 0;
    static const int CLOSED  = 1;
    static const int N_FIXED = 2;

    static const int MAX_ISLAND_OFFSET = 1024;
...
//-- Globals -------------------------
    ...
    static IslandInfo g_islandInfo[MAX_ISLAND_OFFSET];
    static int g_nIslandInfo = 0;
    static OkPieces g_okPieces[N_ROW][N_COL];

    static const int[] g_firstRegion = [
    ...
    static void markBad(IslandInfo info, int mask, int eo, bool always) {
...
//-- Classes -------------------------;
    ...
    static class OkPieces {
        ...
        int pieceVec[N_PIECE_TYPE][N_ORIENT];
    }


    static class IslandInfo {
        int hasBad[N_FIXED][N_PARITY];
        int isKnown[N_FIXED][N_PARITY];
        int alwaysBad[N_PARITY];
    }
    ...
    static class Soln {
        ...
        class SPiece {
            ...
            short row;
            this() {}
            this(int avec, int apiece, int arow) {
                vec = avec;
...
//-----------------------
    ...
    static class Board {
        static const int L_EDGE_MASK = 
                                       ((1 <<  0) | (1 <<  5) | (1 << 10) | (1 << 15) |
                                        (1 << 20) | (1 << 25) | (1 << 30));
        static const int R_EDGE_MASK = L_EDGE_MASK << 4;
        static const int TOP_ROW = (1 << N_COL) - 1;
        static const int ROW_0_MASK =
            TOP_ROW | (TOP_ROW << 10) | (TOP_ROW << 20) | (TOP_ROW << 30);
        static const int ROW_1_MASK = ROW_0_MASK << 5;
        static const int BOARD_MASK = (1 << 30) - 1;
        ...
        static bool badRegion(int[] toFill, int rNew)
        {
        ...
        static int calcBadIslands(int boardVec, int row)
        {
            ...
            bool filled = ((bottom & toFill[0]) == bottom);
        ...
        static bool hasBadIslandsSingle(int boardVec, int row)
        {
            int[] toFill = [~boardVec];
            bool isOdd = ((row & 1) != 0);
            if (isOdd) {
...
//----------------------
    ...
    static class Piece {
        ...
        static const int SKIP_PIECE = 5;

        static const int[] BaseVecs = [
        ...
        static void setCoordList(int vec, ref int pts[N_ELEM][N_DIM]) {
            int iPt = 0;
            int mask = 1;
            for (int y = 0; y < N_ROW; y++) {
                for (int x = 0; x < N_COL; x++) {
        ...
        static int toBitVector(int pts[N_ELEM][N_DIM]) {
            int y, x;
            int result = 0;
        ...
        static void shiftUpLines(ref int pts[N_ELEM][N_DIM], int shift) {
        ...
        static int shiftToX0(ref int pts[N_ELEM][N_DIM], Instance instance, int offsetRow)
        {
            int x, y, iPt;
            int xMin = pts[0][X];
            int xMax = xMin;
